import { getPostEffects, getStats, iwnQ, states } from "../Helper.js";
import { Talent } from "../types";


const TalentMap : Map<string, Talent> = new Map()
TalentMap
//ALBEDO
    .set('Favonious Bladework - Weiss', {
        ...
        formRender() {
            ...
            return iwnQ(`#${this.id}`).index(0) as HTMLDivElement ?? favoniousBladeworkWeiss
        },
        connect() {
        ...
        update() {
            const scalingAtlas: {[key:string]:any} = {
            ...
            const talentLevel = parseFloat((iwnQ(`#${this.id} span#Level input`).index(0) as HTMLInputElement).value) - 1
            const spans = iwnQ(`#${this.id} span:not(#Level)`)

            spans
                .forEach((talent) => {
                    let talentTags : string[] = ['Total']
                    let Bonus : number;
                    let Boost : number;
                    let Special : number;

                    if (talent.id === 'ChargedAttack') {
                        talentTags.push('Total', ...JSON.parse((talent as HTMLElement).dataset.damage_tags as string) as string[])
                        ...
                        const damageScaling = scalingAtlas[talent.id][talentLevel] as number[]
    ...
    .set('Abiogenesis: Solar Isotoma', {
        ...
        formRender() {
            ...
            return iwnQ(`#${this.id}`).index(0) as HTMLDivElement ?? abiogenesisSolarIsotoma
        },
        connect() {
            const level = iwnQ(`#${this.id} #Level input`)
                level.on('change', e => {
    ...
    .set('Rite of Progeniture: Tectonic Tide', {
        ...
        formRender() {
            ...
            return iwnQ(`#${this.id}`).index(0) as HTMLDivElement ?? riteOfProgenitureTectonicTide
        },
        connect() {
            const level = iwnQ(`#${this.id} #Level input`)
                level.on('change', e => {
...
//JEAN
    ...
    formRender() {
        ...
        return iwnQ(`#${this.id}`).index(0) as HTMLDivElement ?? favoniousBladework
    },
    connect() {},
    update() {
...
function getBonus(damageTags : string[]) {
    ...
    Object.entries(Stats)
        .forEach(([key,value]) => {
            if (bonusNames.includes(key)) Bonuses.push(value)
        })
...
function getBoosts(damageTags : string[]) {
    ...
    Object.entries(Stats)
        .forEach(([key,value]) => {
            if (boostsNames.includes(key)) Boosts.push(value)
        })
    Object.entries(PostEffects)
        .filter(([key,value]) => {
            if (boostsNames.includes(key)) Boosts.push(value)
...
function getSpecial(damageTags : string[]) {
    ...
    Object.entries(Stats)
        .forEach(([key,value]) => {
            if (specialsNames.includes(key)) Specials.push(value)
        })
    Object.entries(PostEffects)
        .filter(([key,value]) => {
            if (specialsNames.includes(key)) Specials.push(value)
