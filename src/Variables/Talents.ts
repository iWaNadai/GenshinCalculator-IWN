import EffectsFormComponenet from "../Components/EffectsComponent";
import StatBoardComponent from "../Components/StatsComponent";
...
export default <Talent[]>[
    {
        ...
        connect() {
            (document.querySelector(`#${this.id} #Level input`) as HTMLInputElement).onchange = handler;
            ...
            function handler() {
                const SCALE_ATLAS : {[key:string]:any} = {
                    NormalAttack1 : [
                ...
                const DIV = document.querySelector(`#${id}`) as HTMLElement
                const TLVL = (parseFloat((DIV.querySelector(`#Level input`) as HTMLInputElement).value) - 1 );
                const SHOWN_STATS = (document.querySelector('iwn-statboard') as StatBoardComponent).Final
                const SECRET_STATS = (document.querySelector('iwn-statboard') as StatBoardComponent).SecretBonus

                Array.from(DIV.querySelectorAll('span:not(#Level)'))
                    .forEach(a => {
                        const SPAN = a as HTMLElement
                        const [NORMAL, CRIT] = SPAN.querySelectorAll('p:not(.name)')
                        const TALENT_TAGS = JSON.parse(SPAN.dataset.info as string) as string[];
                            TALENT_TAGS.push(DIV.dataset.infusion as string)
                        ...
                        if (SPAN.id === 'ChargedAttack') {
                            let output = SCALING.map((Scaling : number) => TALENT_CALCULATOR('ATK', Scaling))
                            output = output.map((output : number) => BOOST_BONUS(output, BOOST, BONUS))

                            NORMAL.innerHTML = `${output.map((a : number) => a.toFixed(2)).join(' + ')}`
                            CRIT.innerHTML = `${output.map((a : number) => (a * CRITICAL_MULTIPLIER).toFixed(2)).join(' + ')}`
                            return
                        }
        ...
        disconnect() {
            (document.querySelector(`#${this.id} #Level input`) as HTMLInputElement).onchange = null;
        }
...
function TALENT_CALCULATOR(StatName : string, Scaling : number) : number {    
    const SCALE_STAT = (document.querySelector('iwn-statboard') as StatBoardComponent).Final[types[StatName]]
...
function BOOST_BONUS(InitOutput : number, Boost : number, Bonus : number) {
